...
// short radio briefing, no jargon framing. The "Week of … —" prefix baked into
// the stored title is stripped so it isn't read aloud awkwardly. Handles both
// the legacy ISO form ("Week of 2026-06-15 — ") and the plain-language range
// form ("Week of June 22–28, 2026 — "); the strip runs up to the first SPACED
// em/en-dash, so the non-spaced en-dash inside a range ("22–28") is preserved.
// Events past this rank are read as headline + "why it matters" only. At five
// events the whole briefing was ~2,300 characters; reading fifteen in full
// would be ~7,000, which is both a long listen and a proportional ElevenLabs
// bill. Full detail for the lead stories, a fast run-down for the rest.
...
export function briefingNarration(edition, events) {
  ...
  const title = rawTitle.replace(/^Week of\s+.+?\s+[—–]\s+/i, "").trim();
  const ordinals = [
...
export async function synthesizeBriefing(text, opts = {}) {
  ...
  const res = await fetch(`https://api.elevenlabs.io/v1/text-to-speech/${voiceId}`, {
    method: "POST",
    // Never let Next.js's App Router fetch cache store or replay this response:
    // the URL is the same for every narration (it only carries the voice id), so
    // a cached body would be served for different text. We cache deliberately in
    // Postgres by content hash instead.
    cache: "no-store",
    headers: {
      "xi-api-key": apiKey,
      accept: "audio/mpeg",
