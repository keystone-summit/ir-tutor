...
// `bucketOf` says which field a desk's `buckets` are keyed on — region by
// default; the generator's topic desks pass the quota category instead.
export function mergeDeskPicks(perDesk, target, bucketOf = (p) => p.region_bucket) {
  const chosen = [];
  const leftovers = [];
  ...
  for (const { group, picks } of perDesk) {
    ...
    for (const bucket of group.buckets) {
      ...
      const i = picks.findIndex((p, idx) => !taken.has(idx) && bucketOf(p) === bucket);
      if (i !== -1) taken.add(i);
    }
