...
export function classifyCandidate(row) {
  ...
  if ((region === "MEX" || region === "VEN" || region === "LAC") && scores.americas > 0) scores.americas += 0.5;
...
// ---------------------------------------------------------------------------
// Source tiers — think tanks weighted above news wires
// ---------------------------------------------------------------------------
//
// John's instruction, verbatim: think-tank and research-institution sources get
// "more weight than news feeds". They are the layer governments, oil majors and
// institutional money actually read, and they publish weekly or monthly — so
// under a flat recency sort they are buried by wire copy every week. Adding them
// without weighting them achieves nothing.
//
// The tier is carried ON THE ROW (`row.tier`, set by the generator's SQL from
// lib/seminarFeeds.ANALYSIS_SOURCE_NAMES). This module never hardcodes source
// names: a new feed tagged `tier: "analysis"` in seminarFeeds.js inherits all of
// the behaviour below automatically.

export const ANALYSIS_TIER = "analysis";
export const NEWS_TIER = "news";

/**
 * How much of a head start an analysis row gets in the pool ordering, expressed
 * in days of apparent freshness.
 *
 * Ten days is chosen off John's own example: a Chatham House piece from four
 * days ago must beat a wire story from four hours ago. With a ten-day bonus the
 * Chatham House piece scores as if it were six days in the FUTURE, so it wins
 * comfortably — and any analysis piece up to ten days old still outranks
 * anything on the wire. Past ten days it falls back behind fresh news but stays
 * in the pool, which is the right shape: old analysis is still worth reading,
 * just not ahead of this morning's news.
 */
export const ANALYSIS_RECENCY_BONUS_DAYS = 10;

/**
 * Minimum analysis rows reserved in the pool before anything else is allocated.
 *
 * The recency bonus alone is an ordering rule, and ordering can still be starved
 * if a category sweep spends every slot on news it happens to reach first. This
 * floor is the belt-and-braces: 40 of the 200 pool slots are held for the
 * analysis tier (or as many as exist, if fewer). It is what makes "the think
 * tanks actually appear" a guarantee rather than a hope.
 */
export const ANALYSIS_POOL_FLOOR = 40;

/**
 * Per-source caps.
 *
 * A previous edition ran three of its four Americas items off one source. Real
 * breadth means several institutions agreeing, not one institution repeated, so
 * a source is capped WITHIN a category — hard at 2 on the finished edition, and
 * looser at 6 in the pool so the model and the backfill still have spares to
 * choose between.
 */
export const MAX_PER_SOURCE_PER_BUCKET = 2;
export const POOL_MAX_PER_SOURCE_PER_BUCKET = 6;

const DAY_MS = 86400000;

/** A row's tier, defaulting to news so an untagged row is never promoted. */
export function tierOf(row) {
  return row && String(row.tier) === ANALYSIS_TIER ? ANALYSIS_TIER : NEWS_TIER;
}

/** Feed source name for a pool candidate or a finished item. */
export function sourceOf(x) {
  if (!x) return null;
  const row = x.source_row || x.row || x;
  const s = row && row.source;
  return s ? String(s) : null;
}

function rowTimeMs(row) {
  const raw = row && (row.ts != null ? row.ts : row.published_at != null ? row.published_at : row.fetched_at);
...
/**
 * Effective freshness used to order the pool: real timestamp plus the analysis
 * tier's bonus. Exported so the guard test can assert the weighting directly
 * rather than inferring it from a pool that happened to come out right.
 */
export function poolScoreMs(row) {
  return rowTimeMs(row) + (tierOf(row) === ANALYSIS_TIER ? ANALYSIS_RECENCY_BONUS_DAYS * DAY_MS : 0);
}
 ...
 * caps rows per source; this function then orders by TIER-WEIGHTED freshness,
 * reserves a floor for the analysis tier, reserves slots per required category
 * so every bucket reaches the prompt with enough candidates to actually fill
 * its minimum, and caps both Iran and any single source so neither dominates.
 *
 * `rows` should be ordered newest-first; this function re-orders by
 * `poolScoreMs` anyway, so the caller's order is a tie-break only.
 */
export function buildCandidatePool(rows, opts = {}) {
  const limit = opts.limit || 200;
  const analysisFloor = opts.analysisFloor != null ? opts.analysisFloor : ANALYSIS_POOL_FLOOR;
  const sourceCap =
    opts.poolSourceCap != null ? opts.poolSourceCap : POOL_MAX_PER_SOURCE_PER_BUCKET;

  const classified = (rows || [])
    .map((row, i) => ({ row, bucket: classifyCandidate(row), _i: i }))
    // Tier-weighted freshness. Ties fall back to the caller's order, so a stable
    // input still produces a stable pool.
    .sort((a, b) => poolScoreMs(b.row) - poolScoreMs(a.row) || a._i - b._i);

  const used = new Set();
  const picked = [];
  const bySourceBucket = {};

  const sourceBucketKey = (c) => `${sourceOf(c) || "_"}::${c.bucket || "_"}`;
  const sourceCapped = (c) => {
    if (!c.bucket || !sourceCap) return false; // unclassified rows are not capped
    return (bySourceBucket[sourceBucketKey(c)] || 0) >= sourceCap;
  };
  const take = (c) => {
    used.add(c.row.id);
    bySourceBucket[sourceBucketKey(c)] = (bySourceBucket[sourceBucketKey(c)] || 0) + 1;
    picked.push(c);
  };

  // 1) Reserve a floor for the analysis tier. Held FIRST so the think tanks are
  //    in the prompt before any category sweep or recency fill spends the space.
  if (analysisFloor > 0) {
    let n = 0;
    for (const c of classified) {
      if (n >= analysisFloor || picked.length >= limit) break;
      if (used.has(c.row.id)) continue;
      if (tierOf(c.row) !== ANALYSIS_TIER) continue;
      if (sourceCapped(c)) continue;
      take(c);
      n++;
    }
  }

  // 2) Reserve slots for every bucket with a pool guarantee, required ones
  ...
  for (const b of ordered) {
    ...
    if (!want) continue;
    // Analysis rows already taken in step 1 count toward the guarantee.
    let n = picked.filter((c) => c.bucket === b.key).length;
    for (const c of classified) {
  ...
  // 3) Fill the remainder by tier-weighted recency, respecting each bucket's
  //    pool cap so a heavy news week on one topic cannot swamp the prompt.
  const counts = {};
...
/** How many pool candidates came from each tier — used for edition reporting. */
export function poolTierCounts(pool) {
  const counts = { [ANALYSIS_TIER]: 0, [NEWS_TIER]: 0 };
  for (const c of pool || []) counts[tierOf(c.row)]++;
  return counts;
}
...
export function enforceQuotas(modelItems, pool, opts = {}) {
  ...
  // --- 2b) Per-source cap WITHIN a category. -------------------------------
  //
  //     A previous edition carried three of its four Americas items from one
  //     source. Breadth means several institutions agreeing, not one repeated,
  //     so a source may hold at most MAX_PER_SOURCE_PER_BUCKET slots in any one
  //     category. Lowest-ranked duplicates go first; the freed slots are
  //     refilled from a different source by steps 3 and 5.
  const sourceCap = opts.maxPerSourcePerBucket != null
    ? opts.maxPerSourcePerBucket
    : MAX_PER_SOURCE_PER_BUCKET;
  const sourceBucketCounts = {};
  const sbKey = (src, bucket) => `${src}::${bucket}`;
  if (sourceCap > 0) {
    items = items.filter((it) => {
      const src = sourceOf(it);
      if (!src || !it.bucket) return true; // nothing to attribute — never dropped
      const k = sbKey(src, it.bucket);
      sourceBucketCounts[k] = (sourceBucketCounts[k] || 0) + 1;
      if (sourceBucketCounts[k] <= sourceCap) return true;
      sourceBucketCounts[k]--;
      dropped.push({
        ...it,
        dropped_because: `${src} over per-source cap of ${sourceCap} in ${it.bucket}`,
      });
      return false;
    });
  }
  ...
  // Would adding this candidate breach the per-source cap for that category?
  const overSourceCap = (c, bucketKey) => {
    if (!sourceCap) return false;
    const src = sourceOf(c);
    if (!src || !bucketKey) return false;
    return (sourceBucketCounts[sbKey(src, bucketKey)] || 0) >= sourceCap;
  };
  const noteSource = (c, bucketKey) => {
    const src = sourceOf(c);
    if (!src || !bucketKey) return;
    const k = sbKey(src, bucketKey);
    sourceBucketCounts[k] = (sourceBucketCounts[k] || 0) + 1;
  };
  ...
  const nextCandidate = (bucketKey) => {
    for (const c of poolList) {
      ...
      if (overSourceCap(c, bucketKey)) continue;
      return c;
    }
    return null;
  };
  ...
  while (items.length < total) {
    ...
    for (const c of poolList) {
      ...
      const t = String(c.row.title || "").toLowerCase().trim();
      if (t && usedTitles.has(t)) continue;
      if (overSourceCap(c, c.bucket)) continue;
...
/**
 * Share of a finished edition that came from the analysis tier.
 * Reported on the edition so "did the think tanks actually make it in?" is a
 * number John can read, not something to be inferred from the source names.
 */
export function editionTierCounts(items) {
  const counts = { [ANALYSIS_TIER]: 0, [NEWS_TIER]: 0 };
  for (const it of items || []) counts[tierOf(it.source_row || it.row)]++;
  return counts;
}

/** e.g. "sources: 4 analysis · 11 news". */
export function formatSourceMixLine(items) {
  const t = editionTierCounts(items);
  return `sources: ${t[ANALYSIS_TIER]} analysis · ${t[NEWS_TIER]} news`;
}
 ...
 */
export function validateEdition(counts, shortfalls, total, items) {
  const c = counts || {};
  const problems = [];

  // Per-source cap within a category. Checked here so the guard test fails the
  // build if the cap is ever removed from enforceQuotas.
  if (items && items.length) {
    const seen = {};
    for (const it of items) {
      const src = sourceOf(it);
      if (!src || !it.bucket) continue;
      const k = `${src}::${it.bucket}`;
      seen[k] = (seen[k] || 0) + 1;
      if (seen[k] > MAX_PER_SOURCE_PER_BUCKET) {
        problems.push(
          `${src} has ${seen[k]} items in ${it.bucket} — over the per-source cap of ${MAX_PER_SOURCE_PER_BUCKET}`
        );
      }
    }
  }
