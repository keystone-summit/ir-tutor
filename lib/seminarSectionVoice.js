...
// Decide whether the Weekly Briefing SECTION can be served from the MP3 that
// /api/seminar/voice-briefing already synthesised into seminar_briefing_audio,
// instead of paying ElevenLabs a second time for byte-identical narration.
//
// Both paths build their script with briefingNarration(edition, events) — the
// same function on the same rows — so at fifteen events the briefing section
// used to re-synthesise the very ~7,200 characters the weekly voice job had
// already paid for, the first time anyone pressed play on Briefing.
//
// stored is the seminar_briefing_audio row ({ mp3, char_count, voice_id,
// model_id }) or null. Reuse ONLY when it is genuinely the same narration in
// the same voice:
//   - the row exists and actually carries audio bytes
//   - its char_count equals the text we would synthesise now, so a changed
//     edition (a deepen re-run, a re-rank) still regenerates and the listener
//     never hears stale audio
//   - its voice and model match the current config, where the row records them
//     (rows written before those columns used the defaults, so a blank value is
//     not treated as a mismatch)
// Anything else falls through to the normal per-section synthesis path.
export function briefingReuseDecision({ stored, text, voiceId, modelId } = {}) {
  if (!stored || !stored.mp3 || !stored.mp3.length) return { reuse: false, reason: "no_stored_audio" };
  const len = String(text || "").length;
  if (!len) return { reuse: false, reason: "no_text" };
  if (Number(stored.char_count) !== len) return { reuse: false, reason: "text_changed" };
  if (stored.voice_id && voiceId && stored.voice_id !== voiceId) return { reuse: false, reason: "voice_changed" };
  if (stored.model_id && modelId && stored.model_id !== modelId) return { reuse: false, reason: "model_changed" };
  return { reuse: true, reason: "stored_briefing_audio" };
}
...
// buffers. Chunks are synthesised with BOUNDED CONCURRENCY (not sequentially):
// a long section like Pattern Echoes splits into several chunks, and doing them
// one after another blew past the 60s serverless function limit (504). Running
// a few at once keeps wall-time near the slowest single chunk while staying
// gentle on ElevenLabs rate limits. Order is preserved for clean concatenation.
const SYNTH_CONCURRENCY = 3;

export async function synthesizeSection(text, opts = {}) {
  const synth = opts._synth || synthesizeBriefing; // _synth: test injection hook
  const chunks = chunkForSynthesis(text);
  if (!chunks.length) throw new Error("empty narration");
  if (chunks.length === 1) return synth(chunks[0], opts);
  ...
  async function worker() {
    while (next < chunks.length) {
      const i = next++;
      bufs[i] = await synth(chunks[i], opts);
    }
  }
  const pool = Array.from({ length: Math.min(SYNTH_CONCURRENCY, chunks.length) }, worker);
