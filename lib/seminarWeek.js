...
// publishes ONE edition per week — the only edition of the week since
// 2026-09-21. So the freshest possible edition on any given day is the one whose
...
export function seminarHealth({ latestWeekStart, latestPublishedAt, now = new Date() }) {
  ...
  // CHANGED 2026-09-21 — ONCE-WEEKLY CADENCE.
  //
  // There used to be a mid-week (Thursday) staleness check here: the edition was
  // regenerated every Thursday, and the daily heartbeat self-healed a missed
  // Thursday run. The brief is now published ONCE a week, on Monday, because the
  // think-tank and research sources it leans on publish weekly or monthly — a
  // twice-weekly cadence just forced the system to fill the midweek slot with
  // wire copy between analysis drops.
  //
  // The check is removed rather than disabled: left in place it would have made
  // the heartbeat regenerate the edition every Thursday afternoon, silently
  // restoring the old twice-weekly behaviour no matter what the cron said.
  // `mid_week_due` / `mid_week_stale` are still returned, always false, so any
  // older caller or stored health blob keeps its shape.
  return {
